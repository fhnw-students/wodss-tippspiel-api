insert into `user` (`username`, `email`, `password`, `admin`)
  values
          ('hirsch', 'gerhard.hirschfeld@students.fhnw.ch', '1234', 1),
          ('davu', 'david.heimgartner@students.fhnw.ch', '1234', 1),
          ('ken', 'ken.iseli@students.fhnw.ch', '1234', 0),
          ('hirsch1', 'gerhard.hirschfeld1@students.fhnw.ch', '1234', 1),	          ('davu1', 'david.heimgartner1@students.fhnw.ch', '1234', 1),	          ('ken1', 'ken.iseli1@students.fhnw.ch', '1234', 0),
          ('hirsch2', 'gerhard.hirschfeld2@students.fhnw.ch', '1234', 1),	          ('davu2', 'david.heimgartner2@students.fhnw.ch', '1234', 1),	          ('ken2', 'ken.iseli2@students.fhnw.ch', '1234', 0),
          ('hirsch3', 'gerhard.hirschfeld3@students.fhnw.ch', '1234', 1),	          ('davu3', 'david.heimgartner3@students.fhnw.ch', '1234', 1),	          ('ken3', 'ken.iseli3@students.fhnw.ch', '1234', 0),
          ('hirsch4', 'gerhard.hirschfeld4@students.fhnw.ch', '1234', 1),	          ('davu4', 'david.heimgartner4@students.fhnw.ch', '1234', 1),	          ('ken4', 'ken.iseli4@students.fhnw.ch', '1234', 0),
          ('hirsch5', 'gerhard.hirschfeld5@students.fhnw.ch', '1234', 1),	          ('davu5', 'david.heimgartner5@students.fhnw.ch', '1234', 1),	          ('ken5', 'ken.iseli5@students.fhnw.ch', '1234', 0),
          ('hirsch6', 'gerhard.hirschfeld6@students.fhnw.ch', '1234', 1),	          ('davu6', 'david.heimgartner6@students.fhnw.ch', '1234', 1),	          ('ken6', 'ken.iseli6@students.fhnw.ch', '1234', 0),
          ('hirsch7', 'gerhard.hirschfeld7@students.fhnw.ch', '1234', 1),	          ('davu7', 'david.heimgartner7@students.fhnw.ch', '1234', 1),	          ('ken7', 'ken.iseli7@students.fhnw.ch', '1234', 0),
          ('hirsch8', 'gerhard.hirschfeld8@students.fhnw.ch', '1234', 1),	          ('davu8', 'david.heimgartner8@students.fhnw.ch', '1234', 1),	          ('ken8', 'ken.iseli8@students.fhnw.ch', '1234', 0),
          ('hirsch9', 'gerhard.hirschfeld9@students.fhnw.ch', '1234', 1),	          ('davu9', 'david.heimgartner9@students.fhnw.ch', '1234', 1),	          ('ken9', 'ken.iseli9@students.fhnw.ch', '1234', 0),
          ('hirsch10', 'gerhard.hirschfeld10@students.fhnw.ch', '1234', 1),	          ('davu10', 'david.heimgartner10@students.fhnw.ch', '1234', 1),	          ('ken10', 'ken.iseli10@students.fhnw.ch', '1234', 0),
          ('hirsch11', 'gerhard.hirschfeld11@students.fhnw.ch', '1234', 1),	          ('davu11', 'david.heimgartner11@students.fhnw.ch', '1234', 1),	          ('ken11', 'ken.iseli11@students.fhnw.ch', '1234', 0),
          ('hirsch12', 'gerhard.hirschfeld12@students.fhnw.ch', '1234', 1),	          ('davu12', 'david.heimgartner12@students.fhnw.ch', '1234', 1),	          ('ken12', 'ken.iseli12@students.fhnw.ch', '1234', 0),
          ('hirsch13', 'gerhard.hirschfeld13@students.fhnw.ch', '1234', 1),	          ('davu13', 'david.heimgartner13@students.fhnw.ch', '1234', 1),	          ('ken13', 'ken.iseli13@students.fhnw.ch', '1234', 0),
          ('hirsch14', 'gerhard.hirschfeld14@students.fhnw.ch', '1234', 1),	          ('davu14', 'david.heimgartner14@students.fhnw.ch', '1234', 1),	          ('ken14', 'ken.iseli14@students.fhnw.ch', '1234', 0),
          ('hirsch15', 'gerhard.hirschfeld15@students.fhnw.ch', '1234', 1),	          ('davu15', 'david.heimgartner15@students.fhnw.ch', '1234', 1),	          ('ken15', 'ken.iseli15@students.fhnw.ch', '1234', 0),
          ('hirsch16', 'gerhard.hirschfeld16@students.fhnw.ch', '1234', 1),	          ('davu16', 'david.heimgartner16@students.fhnw.ch', '1234', 1),	          ('ken16', 'ken.iseli16@students.fhnw.ch', '1234', 0),
          ('hirsch17', 'gerhard.hirschfeld17@students.fhnw.ch', '1234', 1),	          ('davu17', 'david.heimgartner17@students.fhnw.ch', '1234', 1),	          ('ken17', 'ken.iseli17@students.fhnw.ch', '1234', 0);


insert into `nation` ( `id`, `tournament_Group`, `code` )
  values  ( 'RUS', 'A', 'ru'),
          ( 'KSA', 'A', 'sa'),
          ( 'EGY', 'A', 'eg'),
          ( 'URU', 'A', 'uy'),
          ( 'POR', 'B', 'pt'),
          ( 'ESP', 'B', 'es'),
          ( 'MAR', 'B', 'ma'),
          ( 'IRN', 'B', 'ir'),
          ( 'FRA', 'C', 'fr'),
          ( 'AUS', 'C', 'au'),
          ( 'PER', 'C', 'pe'),
          ( 'DEN', 'C', 'dk'),
          ( 'ARG', 'D', 'ar'),
          ( 'ISL', 'D', 'is'),
          ( 'CRO', 'D', 'hr'),
          ( 'NGA', 'D', 'ng'),
          ( 'BRA', 'E', 'br'),
          ( 'SUI', 'E', 'ch'),
          ( 'CRC', 'E', 'cr'),
          ( 'SRB', 'E', 'rs'),
          ( 'GER', 'F', 'de'),
          ( 'MEX', 'F', 'mx'),
          ( 'SWE', 'F', 'se'),
          ( 'KOR', 'F', 'kr'),
          ( 'BEL', 'G', 'be'),
          ( 'PAN', 'G', 'pa'),
          ( 'TUN', 'G', 'tn'),
          ( 'ENG', 'G', 'gb-eng'),
          ( 'POL', 'H', 'pl'),
          ( 'SEN', 'H', 'sn'),
          ( 'COL', 'H', 'co'),
          ( 'JPN', 'H', 'jp');

insert into `game_phase` ( `id`, `code` )
  values  ( 1, 'phase.name.group1'),
          ( 2, 'phase.name.group2'),
          ( 3, 'phase.name.group3'),
          ( 4, 'phase.name.round16'),
          ( 5, 'phase.name.quarters'),
          ( 6, 'phase.name.semis'),
          ( 7, 'phase.name.third'),
          ( 8, 'phase.name.finals');

insert into `location` ( `id`, `code`, `utc_diff` )
values  ( 1, 'location.name.Moscow.Luzhniki', 3),
        ( 2, 'location.name.Yekaterinburg.Central', 5),
        ( 3, 'location.name.SaintPetersburg.Krestovsky', 3),
        ( 4, 'location.name.Rostov-on-Don.Rostov', 3),
        ( 5, 'location.name.Samara.Cosmos', 4),
        ( 6, 'location.name.Volgograd.Volgograd', 3),
        ( 7, 'location.name.Sochi.Fisht', 3),
        ( 8, 'location.name.Kazan.Kazan', 3),
        ( 9, 'location.name.Saransk.Mordovia', 3),
        ( 10, 'location.name.Kaliningrad.Kaliningrad', 2),
        ( 11, 'location.name.Moscow.Otkrytiye', 3),
        ( 12, 'location.name.NizhnyNovgorod.Nizhny', 3);

insert into `game` ( `id`, `date`, `host_nation_id`, `guest_nation_id`, `game_phase_id`, `location_id` )
values  ( 1,  '2018-06-14 18:00:00', 'RUS', 'KSA', 1, 1),
        ( 2,  '2018-06-15 17:00:00', 'EGY', 'URU', 1, 2),
        ( 17, '2018-06-19 21:00:00', 'RUS', 'EGY', 2, 3),
        ( 18, '2018-06-20 18:00:00', 'URU', 'KSA', 2, 4),
        ( 33, '2018-06-25 18:00:00', 'URU', 'RUS', 3, 5),
        ( 34, '2018-06-25 17:00:00', 'KSA', 'EGY', 3, 6),
        ( 4,  '2018-06-15 18:00:00', 'MAR', 'IRN', 1, 3),
        ( 3,  '2018-06-15 21:00:00', 'POR', 'ESP', 1, 7),
        ( 19, '2018-06-20 15:00:00', 'POR', 'MAR', 2, 1),
        ( 20, '2018-06-20 21:00:00', 'IRN', 'ESP', 2, 8),
        ( 35, '2018-06-25 21:00:00', 'IRN', 'POR', 3, 9),
        ( 36, '2018-06-25 20:00:00', 'ESP', 'MAR', 3, 10),
        ( 5,  '2018-06-16 13:00:00', 'FRA', 'AUS', 1, 8),
        ( 6,  '2018-06-16 19:00:00', 'PER', 'DEN', 1, 9),
        ( 22, '2018-06-21 16:00:00', 'DEN', 'AUS', 2, 5),
        ( 21, '2018-06-21 20:00:00', 'FRA', 'PER', 2, 2),
        ( 37, '2018-06-26 17:00:00', 'DEN', 'FRA', 3, 1),
        ( 38, '2018-06-26 17:00:00', 'AUS', 'PER', 3, 7),
        ( 7,  '2018-06-16 16:00:00', 'ARG', 'ISL', 1, 11),
        ( 8,  '2018-06-16 21:00:00', 'CRO', 'NGA', 1, 10),
        ( 23, '2018-06-21 21:00:00', 'ARG', 'CRO', 2, 12),
        ( 24, '2018-06-22 18:00:00', 'NGA', 'ISL', 2, 6),
        ( 39, '2018-06-26 21:00:00', 'NGA', 'ARG', 3, 3),
        ( 40, '2018-06-26 21:00:00', 'ISL', 'CRO', 3, 4),
        ( 10, '2018-06-17 16:00:00', 'CRC', 'SRB', 1, 5),
        ( 9,  '2018-06-17 21:00:00', 'BRA', 'SUI', 1, 4),
        ( 25, '2018-06-22 15:00:00', 'BRA', 'CRC', 2, 3),
        ( 26, '2018-06-22 20:00:00', 'SRB', 'SUI', 2, 10),
        ( 41, '2018-06-27 21:00:00', 'SRB', 'BRA', 3, 11),
        ( 42, '2018-06-27 21:00:00', 'SUI', 'CRC', 3, 12),
        ( 11, '2018-06-17 18:00:00', 'GER', 'MEX', 1, 1),
        ( 12, '2018-06-18 15:00:00', 'SWE', 'KOR', 1, 12),
        ( 28, '2018-06-23 18:00:00', 'KOR', 'MEX', 2, 4),
        ( 27, '2018-06-23 21:00:00', 'GER', 'SWE', 2, 7),
        ( 43, '2018-06-27 17:00:00', 'KOR', 'GER', 3, 8),
        ( 44, '2018-06-27 19:00:00', 'MEX', 'SWE', 3, 2),
        ( 13, '2018-06-18 18:00:00', 'BEL', 'PAN', 1, 7),
        ( 14, '2018-06-18 21:00:00', 'TUN', 'ENG', 1, 6),
        ( 29, '2018-06-23 15:00:00', 'BEL', 'TUN', 2, 11),
        ( 30, '2018-06-24 15:00:00', 'ENG', 'PAN', 2, 12),
        ( 45, '2018-06-28 20:00:00', 'ENG', 'BEL', 3, 10),
        ( 46, '2018-06-28 21:00:00', 'PAN', 'TUN', 3, 9),
        ( 16, '2018-06-19 15:00:00', 'COL', 'JPN', 1, 9),
        ( 15, '2018-06-19 18:00:00', 'POL', 'SEN', 1, 11),
        ( 32, '2018-06-24 20:00:00', 'JPN', 'SEN', 2, 2),
        ( 31, '2018-06-24 21:00:00', 'POL', 'COL', 2, 8),
        ( 47, '2018-06-28 17:00:00', 'JPN', 'POL', 3, 6),
        ( 48, '2018-06-28 18:00:00', 'SEN', 'COL', 3, 5);

update game as g set g.host_score = 2, g.guest_score = 1 where g.id = 1;
update game as g set g.host_score = 1, g.guest_score = 1 where g.id = 2;
update game as g set g.host_score = 0, g.guest_score = 4 where g.id = 3;
update game as g set g.host_score = 0, g.guest_score = 1 where g.id = 4;
update game as g set g.host_score = 2, g.guest_score = 2 where g.id = 5;
update game as g set g.host_score = 3, g.guest_score = 1 where g.id = 6;
update game as g set g.host_score = 1, g.guest_score = 1 where g.id = 7;
update game as g set g.host_score = 2, g.guest_score = 3 where g.id = 8;

insert into `tip` ( `id`, `game_id`, `user_id`, `host_score`, `guest_score`, `tipped_host_score_correctly`, `tipped_guest_score_correctly`, `tipped_winner_correctly`, `tipped_balance_and_winner_correctly`, `points` )
values            (  1,     1,        1,          1,      1,			  0,							 1,								 0,							0,										2 ),
                  (  2,     2,        1,          2,		  1,			  0,							 1,								 0,							0,										2 ),
                  (  3,     3,        1,          0,		  2,			  0,							 0,								 1,							0,										10 ),
                  (  4,     4,        1,          3,		  1,			  0,							 1,								 0,							0,										2 ),
                  (  5,     5,        1,          4,		  1,			  0,							 0,								 0,							0,										0 ),
                  (  6,     6,        1,          2,		  3,			  0,							 0,								 0,							0,										0 ),
                  (  7,     7,        1,          1,		  1,			  1,							 1,								 1,							1,										20 ),
                  (  8,     8,        1,          1,		  2,			  0,							 0,								 1,							1,										16 ),
                  (  9,     9,        1,          2,		  0,			  0,							 0,								 0,							0,										0 ),
                  ( 10,    10,        1,          2,		  3,			  0,							 0,								 0,							0,										0 ),
                  ( 11,     1,        2,          1,		  1,			  0,							 0,								 0,							0,										0 ),
                  ( 12,     2,        2,          2,		  1,			  0,							 0,								 0,							0,										0 ),
                  ( 13,     3,        2,          0,		  2,			  0,							 0,								 0,							0,										0 ),
                  ( 14,     4,        2,          3,		  1,			  0,							 0,								 0,							0,										0 ),
                  ( 15,     5,        2,          4,		  1,			  0,							 0,								 0,							0,										0 ),
                  ( 16,     6,        2,          2,		  3,			  0,							 0,								 0,							0,										0 ),
                  ( 17,     7,        2,          1,		  1,			  0,							 0,								 0,							0,										0 ),
                  ( 18,     8,        2,          1,		  2,			  0,							 0,								 0,							0,										0 ),
                  ( 19,     9,        2,          2,		  0,			  0,							 0,								 0,							0,										0 ),
                  ( 20,    10,        2,          2,		  3,			  0,							 0,								 0,							0,										0 ),
                  ( 21,     1,        3,          1,		  1,			  0,							 0,								 0,							0,										0 ),
                  ( 22,     2,        3,          2,		  1,			  0,							 0,								 0,							0,										0 ),
                  ( 23,     3,        3,          0,		  2,			  0,							 0,								 0,							0,										0 ),
                  ( 24,     4,        3,          3,		  1,			  0,							 0,								 0,							0,										0 ),
                  ( 25,     5,        3,          4,		  1,			  0,							 0,								 0,							0,										0 ),
                  ( 26,     6,        3,          2,		  3,			  0,							 0,								 0,							0,										0 ),
                  ( 27,     7,        3,          1,		  1,			  0,							 0,								 0,							0,										0 ),
                  ( 28,     8,        3,          1,		  2,			  0,							 0,								 0,							0,										0 ),
                  ( 29,     9,        3,          2,		  0,			  0,							 0,								 0,							0,										0 ),
                  ( 30,    10,        3,          2,		  3,			  0,							 0,								 0,							0,										0 ),
                (31,2,1,2,1,0,1,0,0,10),
                (32,2,2,2,1,0,1,0,0,6),
                (33,2,3,2,1,0,1,0,0,6),
                (34,2,4,2,1,0,1,0,0,4),
                (35,2,5,2,1,0,1,0,0,6),
                (36,2,6,2,1,0,1,0,0,6),
                (37,2,7,2,1,0,1,0,0,2),
                (38,2,8,2,1,0,1,0,0,7),
                (39,2,9,2,1,0,1,0,0,10),
                (40,2,10,2,1,0,1,0,0,2),
                (41,2,11,2,1,0,1,0,0,9),
                (42,2,12,2,1,0,1,0,0,5),
                (43,2,13,2,1,0,1,0,0,1),
                (44,2,14,2,1,0,1,0,0,8),
                (45,2,15,2,1,0,1,0,0,0),
                (46,2,16,2,1,0,1,0,0,1),
                (47,2,17,2,1,0,1,0,0,8),
                (48,2,18,2,1,0,1,0,0,4),
                (49,2,19,2,1,0,1,0,0,9),
                (50,2,20,2,1,0,1,0,0,9),
                (51,2,21,2,1,0,1,0,0,7),
                (52,2,22,2,1,0,1,0,0,7),
                (53,2,23,2,1,0,1,0,0,4),
                (54,2,24,2,1,0,1,0,0,10),
                (55,2,25,2,1,0,1,0,0,4),
                (56,2,26,2,1,0,1,0,0,6),
                (57,2,27,2,1,0,1,0,0,0),
                (58,2,28,2,1,0,1,0,0,1),
                (59,2,29,2,1,0,1,0,0,3),
                (60,2,30,2,1,0,1,0,0,5),
                (61,2,31,2,1,0,1,0,0,1),
                (62,2,32,2,1,0,1,0,0,0),
                (63,2,33,2,1,0,1,0,0,1),
                (64,2,34,2,1,0,1,0,0,1),
                (65,2,35,2,1,0,1,0,0,7),
                (66,2,36,2,1,0,1,0,0,3),
                (67,2,37,2,1,0,1,0,0,8),
                (68,2,38,2,1,0,1,0,0,7),
                (69,2,39,2,1,0,1,0,0,9),
                (70,2,40,2,1,0,1,0,0,5),
                (71,2,41,2,1,0,1,0,0,4),
                (72,2,42,2,1,0,1,0,0,1),
                (73,2,43,2,1,0,1,0,0,0),
                (74,2,44,2,1,0,1,0,0,3),
                (75,2,45,2,1,0,1,0,0,6),
                (76,2,46,2,1,0,1,0,0,4),
                (77,2,47,2,1,0,1,0,0,2),
                (78,2,48,2,1,0,1,0,0,9),
                (79,2,49,2,1,0,1,0,0,8),
                (80,2,50,2,1,0,1,0,0,6),
                (81,2,51,2,1,0,1,0,0,1);
